<?php
    ...
    if(isset($_GET['del'])) {
    	/*$query = "SELECT ID FROM tblguestbook WHERE ID =" . $_GET['del'];
    	$result = mysql_query($query) or die('Error: ' . mysql_error());
    	echo $_GET['del'] . " " . $result;*/
...
<body>
<?php    
  $query = "SELECT ID, first, last FROM tblguestbook ORDER BY ID";
  $result = mysql_query($query) or die('Error: ' . mysql_error());
?>
    <table align="center">
        <tr>
...
<?php
    while(list($id, $first, $last) = mysql_fetch_array($result, MYSQL_NUM)) {
?>
        <tr>
