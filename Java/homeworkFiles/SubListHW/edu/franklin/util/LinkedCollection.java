...
//     /**
//      * This implementation does not actually do removes or return actual
//      * data from the iterator.  Removal of elements not at the head or
//      * tail require the use of the <tt>priorCursor</tt> field.  Returning
//      * actual data from the iterator will require advancing the two cursor
//      * fields.
//      */
//     private class LinkedCollectionIterator implements Iterator<E>
//     {
//         // the link we have returned data from
//         private Link<E> cursor = null;
//         // the link we have returned data from before the above
//         private Link<E> priorCursor = null;
//         // has the next method been called before remove?
...
//                
//             /*# insert your code here to advance the iteration */
//             
...
// 
//             /*# insert your code here to actually unlink the node */
//             
...
//     }
    ...
    private class LinkedCollectionIterator implements Iterator<E>
    {
        ...
        public void remove()
        {
            ...
            // removing head
            if (this.cursor == LinkedCollection.this.head)
            {
                LinkedCollection.this.head = LinkedCollection.this.head.next;
                // head is also tail, i.e. only one element in list
                if (this.cursor == LinkedCollection.this.tail)
                {
                    LinkedCollection.this.tail = null;
                }
            }
            // removing tail only
            else if (this.cursor == LinkedCollection.this.tail)
            {
                LinkedCollection.this.tail = this.priorCursor;
                this.priorCursor.next = null;
            }
            // removing middle link
            else
            {
                this.priorCursor.next = this.cursor.next;
            }
