...
public class ABCRentalsTest extends junit.framework.TestCase
{
    ...
    /**
     * test all items available
     */
    public void testOrderInStock()
    {
        Furniture f = new Furniture();
        DVDPlayer dvd = new DVDPlayer();
        Television tv = new Television();
        ...
        weeklyOrder.add(dvd);
        weeklyOrder.add(tv);
        //test order was processed
        assertEquals(true, inventory.processOrder(weeklyOrder));
    }
    
    /**
     * test handleOrder, were all items 
     * marked as rented
     */
    public void testHandleOrder()
    {
        ...
        weeklyOrder.add(dvd);
        weeklyOrder.add(tv);
        inventory.processOrder(weeklyOrder);
        //test f was marked as rented
        assertEquals(true, f.isRented());
        //test dvd was marked as rented
        assertEquals(true, dvd.isRented());
        //test tv was marked as rented
        assertEquals(true, tv.isRented());
    }
    
    /**
     * test one item not available
     */
    public void testNotAvailableItem()
    {
        Furniture f = new Furniture();
        DVDPlayer dvd = new DVDPlayer();
        Television tv = new Television();
        ...
        weeklyOrder.add(dvd);
        weeklyOrder.add(tv);
        //test order was not processed
        assertEquals(false, inventory.processOrder(weeklyOrder));
        //test order was added to backOrders
        assertEquals(1, inventory.getBackOrders().size());
    }
    
    /**
     * test sortBackOrders
     */
    public void testSortBackOrders()
    {
        Furniture f = new Furniture();
        DVDPlayer dvd = new DVDPlayer();
        Television tv = new Television();
        ...
        monthlyOrder.add(tv);
        Order oneWeekOrder = new Order("123", "Diana", "3305556677", 1);
        oneWeekOrder.add(f);
        oneWeekOrder.add(dvd);
        oneWeekOrder.add(tv);
        //process first order not in stock
        assertEquals(false, inventory.processOrder(weeklyOrder));
        //test it was added to backOrders
        assertEquals(1, inventory.getBackOrders().size());
        //process second order not in stock
        assertEquals(false, inventory.processOrder(monthlyOrder));
        //test it was sorted correctly - added to beginning
        assertEquals(inventory.getBackOrders().getFirst(), monthlyOrder);
        //process third order not in stock
        assertEquals(false, inventory.processOrder(oneWeekOrder));
        //test it was sorted correctly - added to end
        assertEquals(inventory.getBackOrders().getLast(), oneWeekOrder);
    }
    
    /**
     * test checkBackOrders
     */
    public void testCheckBackOrders()
    {
        //no backOrders
        assertEquals(null, inventory.checkBackOrders());
        //add orders to backOrders
        Furniture f = new Furniture();
        DVDPlayer dvd = new DVDPlayer();
        Television tv = new Television();
        inventory.add(f);
        ...
        oneWeekOrder.add(tv);
        inventory.processOrder(weeklyOrder);
        inventory.processOrder(monthlyOrder);
        inventory.processOrder(oneWeekOrder);
        //no backOrdersrders can be filled
        assertEquals(null, inventory.checkBackOrders());
        //return dvd so a backOrder can be filled
        dvd.returned();
        //monthlyOrder can be filled
        assertEquals(monthlyOrder, inventory.checkBackOrders());
    }
    
    /**
     * test processBackOrders
     */
    public void testProcessBackOrders()
    {
        //test process null order
        assertEquals(false, inventory.processBackOrder(null));
        //add orders to backOrders
        Furniture f = new Furniture();
        DVDPlayer dvd = new DVDPlayer();
        Television tv = new Television();
        ...
        inventory.processOrder(monthlyOrder);
        inventory.processOrder(oneWeekOrder);
        //test backOrders have been added
        assertEquals(3, inventory.getBackOrders().size());
        //test filling a backOrder
        assertEquals(true, inventory.processBackOrder(weeklyOrder));
        //test weeklyOrder was removed from backOrders
        assertEquals(2, inventory.getBackOrders().size());
    }
}